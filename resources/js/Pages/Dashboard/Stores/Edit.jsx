...
// Edit Page
// ---------------------------------------------------------------------------
export default function Edit({ store, categories }) {
    ...
    const submit = (e) => {
        ...
        post(route("stores.update", store.id), {
            onSuccess: () => toast.success("Data toko diperbarui! ✨"),
    ...
    const selectedCategory = categories.find(c => c.id === data.store_category_id) ?? null;
        ...
        <>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Edit Data Toko</h1>
                        <p className="text-sm text-slate-500 dark:text-slate-400 font-mono">Kode: {store.code}</p>
                    </div>
                </div>
                            ...
                            {!data.store_category_id && (
                                <div className="mt-4 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                                    <p className="text-xs text-blue-700 dark:text-blue-300">
                                        ℹ️ Tanpa kategori, toko ini dapat menjual <strong>semua variant</strong> yang aktif.
                                    </p>
                                </div>
                            )}

                            {selectedCategory && (
                                selectedCategory.allow_all_variants ? (
                                    <div className="mt-4 p-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                                        <p className="text-xs text-green-700 dark:text-green-300">
                                            ✅ Kategori <strong>{selectedCategory.code}</strong> mengizinkan semua variant dijual di toko ini.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="mt-4 p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                                        <p className="text-xs text-amber-700 dark:text-amber-300">
                                            ⚠️ Kategori <strong>{selectedCategory.code}</strong> menggunakan whitelist variant.
                                            Hanya variant yang dikonfigurasi di <strong>Master Kategori</strong> yang akan tampil di POS toko ini.
                                        </p>
                                    </div>
                                )
                            )}
                        ...
                        {categories.length > 0 && (
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
                                ...
                                <div className="space-y-2">
                                    {categories.map(cat => (
                                        <button
                                            key={cat.id}
                                            type="button"
                                            onClick={() => setData("store_category_id", data.store_category_id === cat.id ? "" : cat.id)}
                                            className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-left transition-all ${
                                                data.store_category_id === cat.id
                                                    ? "bg-violet-100 dark:bg-violet-900/40 ring-1 ring-violet-400 dark:ring-violet-600"
                                                    : "bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800"
                                            }`}
                                        >
                                            <div className="flex items-center gap-2">
                                                <span className={`text-xs font-bold font-mono w-6 ${
                                                    data.store_category_id === cat.id
                                ...
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-3 text-center">
                                    Klik kategori untuk memilih / membatalkan pilihan
                                </p>
                            </div>
                        )}
                    </div>
